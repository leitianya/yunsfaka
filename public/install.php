...
// 安装包目录
define('INSTALL_PATH', ROOT_PATH . 'install' . DS . 'command' . DS . 'Install' . DS);
...
if (is_file($lockFile))
{
    $errInfo = "当前已经安装{$sitename}，如果需要重新安装，请手动移除install/command/Install/install.lock文件";
}
...
if (!$errInfo && isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST')
{
    ...
    {
        //检测能否读取安装文件
        $sql = @file_get_contents(INSTALL_PATH . 'daishua.sql');
        if (!$sql)
        {
            throw new Exception("无法读取install/command/Install/daishua.sql文件，请检查是否有读权限");
        }
        ...
        if (!$result)
        {
            throw new Exception("无法写入安装锁定到install/command/Install/install.lock文件，请检查是否有写权限");
        }
        $newPassword = md5('yunshangfaka2017'.$adminPassword);
        $pdo->query("UPDATE ys_admin SET uname = '{$adminUsername}',passwd = '{$newPassword}'WHERE id = 1");
        echo "success";
    }
    catch (Exception $e)
    {
                    ...
                    $.post('', $(this).serialize())
                        .done(function (ret) {
                            if (ret === 'success') {
                                ...
                                $('<a class="btn" href="/">访问首页</a> <a class="btn" href="/ysadmin.html" style="background:#18bc9c">访问后台</a>').insertAfter($button);
                                $button.remove();
                            } else {
                                $('#error').show().text(ret);
